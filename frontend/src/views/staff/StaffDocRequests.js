...
import { deleteDocumentRequest, loadDocumentsRequests } from "../../store/documentsRequests/documentsRequests";
import { withRouter } from "react-router-dom";


function StaffDocRequests(props) {
    const { classes } = props;
    ...
    const getCount = () => {
        const currentUserRequest = documentRequests.filter(req => req.documentType === "refrenceLetter");
    ...
    const renderRequests = () => {
        ...
        const list = pendeingReuests.slice(
            ...
            <TableRow key={req._id} component={Paper} className={classes.infoDataRow}>
                ...
                <TableCell align="center">Refrence Letter</TableCell>
                <TableCell align="right">
                    <Tooltip arrow title="Approve" classes={toolTipClasses}>
                        <IconButton
                            onClick={
                                () => {
                                    props.history.push(`/staff/documents-requests/${req._id}`);
                                }
                            }
                        >
                            <ApproveIcon />
    ...
    const getAppointmentInfo = () => {
        ...
        return "Request info can't be found :("
    };
    ...
    const handleAlertYes = () => {
        if (action === "delete") {
            dispatch(deleteDocumentRequest(reqToBeActedOn)).then(
                () => {
                    if (isError) {
                        ...
                        setSnackMsg("Request Request Deleted Successfully!");
                        setSnack(true);
                        return;
                    }
                }
    ...
    return (
        <div>
            ...
            <AlertMesssageDialog
                values={{
                    openAlert: alert,
                    title: `Do you really want to ${action === "delete" ? "Delete" : "grant"} this Request?`,
...
export default withRouter(withStyles(styles)(StaffDocRequests));