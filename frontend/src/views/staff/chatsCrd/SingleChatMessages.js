...
function SingleChatMessages(props) {
    ...
    const getCurrentChat = () => {
        ...
        if (chats[index]) return chats[index]
    };
    ...
    const getRecieverMessages = (id, recieverId) => {
    ...
    const getConcernedChats = () => {
        if (Object.keys(currentUser).length !== 0) {
            ...
            const recieverMessages = getRecieverMessages(currentUser._id, currentChat.sender);
            const senderMessages = getSenderMessages();
    ...
    return (
        <div>
            {
                (currentChat && !isLoading && Object.keys(currentChat).length !== 0) && <>
...
export default withStyles(styles)(SingleChatMessages);